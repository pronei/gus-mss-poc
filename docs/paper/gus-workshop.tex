...
For the sequence above the ledger output is (the tool's own text, long
lines wrapped, arrow and dash glyphs transliterated):

{\footnotesize\begin{verbatim}
identity "shipment-tracking" -- EXPOSED
  born     @ E03: checkout provides it as string on
                  field "shipment_ref" (required=true, ...)
  eroded   @ E07: field "shipment_ref" went
                  required->optional at checkout -- the
                  identity is no longer guaranteed present
  demanded @ E11: now required by [email] -- the
                  proposal was REJECTED because the
                  guarantee no longer holds
  violated @ E11: chain check fails (chain-weakened)
                  -- guarantee last weakened at step "E07"
\end{verbatim}}

\noindent E11's batch is correctly blocked; E07 is correctly
...
Step & Outcome & Notes\\
\midrule
E01 wishlist & ships & additive response fields\\
E02 cart & blocked & response guarantee regressed\\
\textbf{E03 accounts} & \textbf{ships in 2 stages} & callers first, then shipping\\
E04 payments & blocked & response enum widened; nullable id\\
\textbf{E05 payments} & \textbf{ships in 2 stages} & checkout before payment\\
E06 money, expand & partial & widening ships as a warning; a map value change is blocked\\
\textbf{E07 money, contract} & \textbf{ships in 2 stages} & four violation kinds, one order; quietly weakens a guarantee\\
\textbf{E08 tracing} & \textbf{partial} & rename without alias: only the chain fails\\
E09 tracing & ships & alias bridges the rename\\
\textbf{E10 promo} & \textbf{blocked} & every mixed pair passes; only the target pair conflicts\\
\textbf{E11 notifications} & \textbf{blocked} & E07's weakening exposed; ledger names E07\\
\bottomrule
\end{tabular}
\end{table}

\emph{Order (E03).} Shipping begins requiring an
...
\emph{Target state (E10).} Shipping accepts an optional
...
\emph{History (E07 and E11).} As in \S\ref{sec:ledger}: the
weakening ships silently at E07 because nothing requires the
identity; the requirement arrives at E11 and is blocked; the
ledger names E07. Without the ledger, E11's failure is
