...
| E08 `tracing-first-cut` | Frontend originates `session-trace`; checkout forwards it *renamed*, no alias; email requires it | **partial** | Every per-edge conjunct passes — only `chain-field-missing` fires; the solver keeps the harmless middle hop and conservatively drops both ends |
| E09 `tracing-aliased` | Checkout declares `x-alias: client_session_id` | **ships** | The rename bridge (deck slide 9, realized): the chain verifies through the intermediate hop |
